...
//     o2ws_on_error(msg) (externally defined) - error messages (strings)
//         are passed to this function. This function is not defined here,
...
var o2ws_message_bytes = "";  // queue of incoming byte stream
var o2ws_message_fields = [];  // all received message fields
var o2ws_message = [];  // message fields of a single message obtained from
                        // o2ws_message_fields
...
function o2ws_message_handler(evt) {
    ...
    o2ws_message_bytes += evt.data
    if (o2ws_message_bytes.slice(-1) != ETX) {
        if (o2ws_status_msgs_enable) {
            o2ws_status_msg("    INCOMPLETE FIELD");
        }
        return;  // wait at least for the rest of the last field
    }
    var new_fields = o2ws_message_bytes.split(ETX);    // parse the new stuff
    o2ws_message_bytes = ""
    new_fields.pop();  // remove empty string at the end due to final ETX
    o2ws_message_fields = o2ws_message_fields.concat(new_fields)
    if (o2ws_status_msgs_enable) {
        o2ws_status_msg("PENDING FIELDS: " + o2ws_message_fields);
    }
    while (true) {  // parse and send messages from o2ws_message_fields
        // sanity check: message has address, typespec, timestamp, flag and
        // ends with ETX
        if (o2ws_message_fields.length < 4) {
            return;
        }
        // find a handler
        var address = o2ws_message_fields[0];
        // sanity check: address must begin with ! or /:
        if (address.charAt(0) != "!" && address.charAt(0) != "/") {
            o2ws_on_error("Unexpected O2 address field: " + address);
            // we are probably unable to proceed because something is
            // out of sync, but we'll continue anyway.
        }
        address = address.substring(1);  // remove first ! or /
        ...
        if (o2ws_message_fields.length < typespec.length + 4) {
            return;  // wait for more fields to arrive
        }
        // extract the first message from o2ws_message_fields:
        o2ws_message = o2ws_message_fields.splice(0, typespec.length + 4)
        if (o2ws_status_msgs_enable) {
            o2ws_status_msg("MESSAGE: " + o2ws_message + 
                            " REMAINING FIELDS: " + o2ws_message_fields);
        }
        ...
        if (o2ws_method_dict.hasOwnProperty(address)) {
            ...
            o2ws_message.splice(0, 4);  // remove address,timestamp,typespec,tcp
            ...
            for (h of o2ws_method_array) {
                if (address.startsWith(h.address) && 
                    ...
                    if (h.typespec === null || h.typespec == typespec) {
                        // leaves only parameters:
                        o2ws_message_fields.splice(0, 4);  
                        o2ws_schedule_handler(h.handler, timestamp,
                                              h.address, typespec, h.info);
...
function o2ws_get_float() {
    var s = o2ws_message.shift();  // get the next parameter
    return parseFloat(s);
}
...
function o2ws_get_string() {
    return o2ws_message.shift();  // get the next parameter
}
