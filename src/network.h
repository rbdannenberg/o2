...
// net_tag values
#define NET_UDP_SERVER     50   // receives UDP messages
#define NET_TCP_SERVER     51   // server port for TCP connections
#define NET_TCP_CONNECTING 52   // client side socket during async connection
#define NET_TCP_CLIENT     53   // client side of a TCP connection
#define NET_TCP_CONNECTION 54   // server side accepted TCP connection
#define NET_INFO_CLOSED    55   // o2n_close_socket() has been called on this
...
typedef struct o2n_address {
    ...
    int port; // for convenience, make the port accessible here
} o2n_address, *o2n_address_ptr;


extern char o2n_local_ip[24];
extern int o2n_local_tcp_port;
extern int o2n_found_network; // true if we have an IP address, which implies a
