                 ...
                 o2_arg_ptr *argv, int argc, const void *user_data)
{
    ...
    sync_value = argv[0]->i;
    printf("[service_one: /one/sync %d]", sync_value);
}
...
{
    printf("* Sending /two/sync %d, waiting for %d: ", i, i); fflush(stdout);
                 ...
                 o2_arg_ptr *argv, int argc, const void *user_data)
{
    ...
    if (streql(service_name, "_cs") || streql(service_name, "_o2")) {
        ...
        if (last_sync == 0 || last_sync == 1) correct = "attr1:value1;";
            ...
            assert(streql(properties, "attr2:value2;attr1:value1;") ||
...
int main(int argc, const char * argv[])
{
    ...
    o2_service_set_property("one", "attr0", "onevalue1one"); // will match value1
    assert(o2_services_list_free() == O2_SUCCESS);
